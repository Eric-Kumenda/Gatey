...
import SaveAsCustomGateModal from "./SaveAsCustomGateModal";
...
const Workspace = () => {
  ...
  const handleClick = (e) => {
    if (connectionInProgress?.isConnecting) {
      ...
      const isPort =
        clickedElement.classList.contains("gate-port") ||
        clickedElement.classList.contains("panel-port");
  ...
  const handlePortClick = useCallback(
    (clickedGateId, clickedPortId, clickedPortType, portElement) => {
      if (connectionInProgress?.isConnecting) {
        ...
        if (sourceGateId === "inputPanel") {
          ...
          if (!sourceGate || !sourceGate.ports) {
            console.error(
              "Source gate or its ports not found in state:",
              sourceGateId
            );
            dispatch(
              setConnectionInProgress({
        ...
        let finalSourceGateId,
          finalSourcePortId,
          finalDestinationGateId,
        ...
        if (sourcePortType === "output" && clickedPortType === "input") {
          ...
          console.warn(
            "Invalid connection type. Cannot connect input to input or output to output."
          );
          dispatch(
            setConnectionInProgress({
              isConnecting: false,
        ...
        dispatch(
          setConnectionInProgress({
            sourceGateId: clickedGateId,
            sourcePortId: clickedPortId,
            isConnecting: true,
          })
        );
      }
    },
    [connectionInProgress, gates, dispatch]
  ); // Dependencies for useCallback

  // This function is passed to the Wire component to find actual DOM elements
  const getGateAndPortElements = useCallback(
    (gateId, portId) => {
      if (gateId === "mouse" && portId === "current") {
        return {
          ...
          portElement: {
            getBoundingClientRect: () => ({
              ...
              top:
                mousePosition.y +
                (workspaceRef.current?.getBoundingClientRect().top || 0),
              width: 0,
              height: 0,
            }),
          },
        ...
        const portElement = document.getElementById(
          `gate-${gateId}-port-${portId}`
        );
        return { gateElement, portElement };
      }
    },
    [mousePosition]
  ); // mousePosition is a dependency for the temporary wire

  return (
    <>
      <div className="d-flex h-100 position-relative">
        {" "}
        {/* Flex container for panels and workspace */}
        <InputPanel onPortClick={handlePortClick} /> {/* Render InputPanel */}
        <div
          ref={attachRef}
          id="workspace"
          className={`d-flex flex-column position-relative overflow-x-scroll rounded border h-100 flex-grow-1 bg-${backgroundColor}`}
        ...
        <button
          className="btn btn-outline-primary position-absolute shadow top-0 start-50 mt-5 mx-auto"
