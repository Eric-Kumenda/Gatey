...
const Gate = ({ id, type, initialPosition, onPortClick }) => {
  ...
  const getGatePorts = (gateType, gateData) => {
    // If it's a custom gate, extract port definitions from the input/output panel connections
    if (gateData?.inputs && gateData?.outputs) {
      return {
        inputs: gateData.inputs.map((input) => ({
          id: input.id,
          label: input.name || "IN",
          type: "input",
        })),
        outputs: gateData.outputs.map((output) => ({
          id: output.id,
          label: output.name || "OUT",
          type: "output",
        })),
      };
    }
  ...
  const gatePorts = getGatePorts(type, gate);
